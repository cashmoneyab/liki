...
credits to 
Anna W. Anna W. <https://devforum.roblox.com/u/ImActuallyAnna> Skylar L. <https://devforum.roblox.com/u/ScobayDu>
https://github.com/rce-incorporated/Fiu for the interpreter


for the liki project

an easy and accessible, open-source way to pause / play roblox games

features :
	real - time, resume and pause scripts and also physics, tweens, animations, etc
	open - source engineering, so that why you can add your own compiler and interpreter
	
	and super easy to use
	
btw, you can use "create liki", as a shortcut, check Advanced -> Customize Shortcuts and search for liki.
]]
...
if RunService : IsRunning () and RunService : IsServer () then return; end;
...
local function collectBinary (url : string)
	local rawSource = HttpService : GetAsync(url);
	local source = "";	rawSource : gsub("\\(%d+)", function (c) source = source .. string.char (c) end)
	return source;
end

local function getRBXMreader () : (binary : string) -> Instance
	local source = HttpService : GetAsync("https://raw.githubusercontent.com/cashmoneyab/liki/refs/heads/main/rbxmreader.lua");
	local module = loadstring (source) ()
	if module then
		return module.ReadRBXM;
	end
	error ('rbxmreader couldn\'t load');
end

local function ungroup (model : Model)
	if model then
...
local button1, button2 = toolbar : CreateButton ("Open Menu for LIKI", "openMenuLiki", "rbxassetid://12967351548", "Open Menu for LIKI"), toolbar : CreateButton ("Setup Simulation", "setupLiki", "rbxassetid://12974220219", "Setups the LIKI simulation")
...
button2.Click : Connect (function ()
	xpcall(function ()
		if loader then
			...
			local read = getRBXMreader ();
			local m = read (collectBinary ("https://raw.githubusercontent.com/cashmoneyab/liki/refs/heads/main/likiloader.binary"));
			loader = require (ungroup (m) [1]);
			loader ();
		end
		setWaypoint();
	end, function (a)
